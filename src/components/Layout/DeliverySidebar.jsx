...
const DeliverySidebar = ({ isOpen, setIsOpen }) => {
  const { user, logout } = useAuth();

  const navigation = [
    { name: 'Orders', href: '/delivery/orders', icon: Package },
    { name: 'Payments', href: '/delivery/payments', icon: CreditCard },
    { name: 'Profile', href: '/delivery/profile', icon: User },
    { name: 'Statistics', href: '/delivery/statistics', icon: BarChart3 },
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-strong transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
